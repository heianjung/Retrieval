...
def calculate_candidates_ranking(prediction, ground_truth, eval_candidates_num=10):
	total_num_split = int(len(ground_truth) / eval_candidates_num)
	...
	print(f" pred_split: ({len(pred_split)}, {pred_split[0].shape})")
	print(f" gt_split: ({len(gt_split)}, {gt_split[0].shape})")

	print(f"before split orig_rank_split: {np.tile(np.arange(0, eval_candidates_num), int(total_num_split))}")
	print(f"eval_candidates_num: {eval_candidates_num}")
	print(f"total_num_split: {total_num_split}")

	print(f" orig_rank_split: ({len(orig_rank_split)}, {orig_rank_split[0].shape})")
