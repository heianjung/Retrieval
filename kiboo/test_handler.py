...
class KiboDocRecommendHandler(BaseHandler):
    ...
    def _encode(self, texts: List[str],
        ...
        embedding = self.model(texts, batch_size=512)
    ...
    def _get_model(self, model_dir: str, serialized_file: str, model_config: str, device: Optional[str] = None):
        ...
        with open(model_dir + '/KRSentenceBERT0.pkl','rb') as intp:    
    ...
    def _get_faiss_index(self, dim: int = 768, device: Optional[str] = None) -> None:
        ...
        for cat in cat_embeddings:
            ...
            self._logger.info(f"Indexing Finished at the category '{cat}': {embeddings.shape}")
        ...
        if device == "gpu" or self.device.type.startswith("cuda"):
            ...
            self._logger.info(f'self.device : {self.device}')
            self._logger.info(f'device : {device}')
            self._logger.info(f'torch.cuda.is_available() : {torch.cuda.is_available()}')
            self._logger.info(f'faiss.get_num_gpus() : {faiss.get_num_gpus()}')
    ...
    def initialize(self, context: Any) -> None:
        if context == 'poc_inference' or context == 'poc_explanation':
    ...
    def delete_file(self, delete_file_path, deleted: List[dict], batch_size:int = 32) -> List[JSON]:
        ...
        self._logger.info("Add File Loaded From {0}".format(delete_file_path))
    ...
    def request_decode(self, data: List[Union[dict, JSON]]) -> List[dict]:
        data = [d['body'] if isinstance(data[0], str) else d['body'].decode(encoding='utf-8') for d in data]
        return [json.loads(d) for d in data]

    def handle(self, data: List[Any], context: Any) -> List[JSON]:
        try : #torchserve
            data = self.request_decode(data)
        except : #local debugging
            data = [data]
        #################

        try:
            response = None
            if context == 'poc_inference':
            ...
            elif context == 'poc_explanation':
                # data[0] = eval(data[0])
                mode = data[0]["mode"]
                print('##################')
                print(mode)
                batch_size = data[0].get("batch_size", 32)
                init_batch_size = data[0].get("batch_size", 32)
                ...
                # @hijung 12.12
                    ...
                    add_file_path = data[0].get("add_file", "") 
                    self._logger.info("Call Document Add File API")
                    response = self.add_file(add_file_path, data, batch_size=min(self.inference_batch_size, batch_size))
                # @hijung 12.12
                elif mode == "delete_file":
                    delete_file_path = data[0].get("delete_file", "")
                    ...
                    self.drop_out_p = model_config['model_config']['dropout_p']
                    ...
                    elif (mode == "indexing") or (mode == "indexing_deploy"):
...
if __name__ == "__main__":
    ...
    kibo = KiboDocRecommendHandler()
    kibo.initialize(context='poc_explanation')
    ###########################################################
    # input
    ...
    # data
    data_dir = "/home/wisenut/kibo_test/deployment/data_dir/small_data" 
    emb_file = 'emb.pickle'
    ...
    config_dict =  {
        "ckpt_dir": ckpt_dir,
        "model_config": "model_config.json", #model_config_test_1201
        "ckpt_file": ckpt_file,
        "data_dir": data_dir,
        "emb_file": emb_file,
        "txt_file": txt_file
    ...
    print("======== 0. server init test ========")
    data = {
    "mode": "server_init",
    "config": config_dict
    }
    pp.pprint(kibo.handle(data, context='poc_explanation'))
    ...
    # }
    # }
    # response = requests.post('http://127.0.0.1:9090/explanations/demo', data=data)
    ...
    # "documents": [
    #     {   
    #         "doc_id": '1020100036011,patentTrust,1',
    #         "document": "평형기능 보완용 두피 접촉식 진동"
    #     },
    #     {
    ...
    #         구체적으로 top gate OTFT에 다양한 고분자절연체를 적용시켜서 계면의 화학 및 물리적 특성이 top gate OTFT의 성능에 미치는 영향에 대해서 연구하고 반도체/절연체 계면에 \
    #         인위적으로 수분, 산소, 이온, 금속촉매 등을 정량적으로 첨가하여 이들의 농도에 따른 OTFT의 특성변화를 관찰하여 환경적 요인에 따른 OTFT 의 특성 및 안정성의 영향에 대해서 체계적으로 연구하고자 한다. O 1차년도: 고분자절연체 및 용매 종류에 따른 top gate OTFT 의 특성변화 연구 O 2차년도: 다양한 환경적 요인에 따른 top gate OTFT의 특성변화 연구 O 3차년도: 다양한 환경적 요인을 monitoring 하기 위한 dual gate OTFT 개발"
    #     }
    # ]
    # }

    # pp.pprint(kibo.handle(input_q, context='poc_inference'))
