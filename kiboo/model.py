...
from transformers import AutoModel, T5Config, AutoConfig, AutoTokenizer
...
class TransformerV2(nn.Module):
    ...
    def load(input_path: str):
        ...
        return TransformerV2(model_name_or_path=input_path, **config)
...
class KRSentenceBERT(nn.Module):
    def __init__(
            self,
            sentence_transformers_path: str,
            hidden_size: int = 768,
        ...
        super().__init__()
        self.sbert = SentenceTransformerV2(sentence_transformers_path, device=device)
    ...
    def forward(self, sentences: Union[str, List[str]], batch_size: int = 32) -> Tensor:
        embedding = self.sbert.encode(sentences, convert_to_tensor=True, batch_size=batch_size)
...
class KRSentenceBERT0(nn.Module):
    ...
    def forward(self, sentences: Union[str, List[str]], batch_size: int = 32) -> Tensor:
        ...
        embedding = self.sbert.encode(sentences, convert_to_tensor=True, batch_size=batch_size, device = self.device)
        return embedding
